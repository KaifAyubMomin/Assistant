...
public class UserInfoController {
    ...
    @GetMapping("/{message}")
    public MessageDTO sample(@PathVariable("message") String message){
        return reply(new MessageDTO(2 , message));
    }

    @PostMapping("/api")
    public MessageDTO reply(@RequestBody MessageDTO messageDTO) {
        System.out.println("message is here :"+ messageDTO.getMessage());
