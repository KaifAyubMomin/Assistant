...
//            }
            ...
            if (matchCount == totalExpectedScore) {
                perMatch ++;
                bestResponse = bestResponse  + (perMatch > 0 ?" and ": " ") +  singleResponse.getResponse();

            }

            if (matchCount > highestMatchCount) {
                highestMatchCount = matchCount;
                bestResponse = singleResponse.getResponse();
            }

            Accuracy = (matchCount * 100) / totalExpectedScore;
            System.out.println("Accuracy  : " + getAccuracyBar(Accuracy) + matchCount + bestResponse);

        }


        return bestResponse != null
                && Accuracy >= 80 ? new MessageDTO(requestId, bestResponse + " Is it correct " + username + "?  "  + "Accuracy here : " + getAccuracyBar(Accuracy))
                : null;
    }
    ...
    public MessageDTO getAllData(MessageDTO messageDTO) {
      ...
      if(sample.isEmpty()){
        ...
        sample.forEach(one -> {
            ...
            responses.append(one.getResponse()).append(" at ");
            responses.append(one.getDetectedTime()).append("    And   ");
    ...
    public MessageDTO getNextWork(MessageDTO messageDTO){
        ...
        if(getSample.isEmpty()){
         ...
         System.out.println("Detection : " + getSample.get().getResponse() + " at "+ getSample.get().getDetectedTime());
        return new MessageDTO(messageDTO.getId() , getSample.get().getResponse() + " at "+ getSample.get().getDetectedTime());
    }
